...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		ShowOwnerKitties {
			owner: T::AccountId,
			kitties: BoundedVec<[u8; 16], T::MaxKittiesOwned>,
		},

		ShowKitties {
			kitties: Vec<([u8; 16], Option<BalanceOf<T>>)>,
		},
	}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight(0)]
		pub fn display_kitties_owner(origin: OriginFor<T>) -> DispatchResult {
			// Make sure the caller is from a signed origin
        ...
        #[pallet::weight(0)]
        pub fn display_kitties(origin: OriginFor<T>) -> DispatchResult {
            // Make sure the caller is from a signed origin
			let _ = ensure_signed(origin)?;
			// Transfer the kitty from seller to buyer as a sale.
			Self::do_display_kitties()?;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn do_display_kitties_owner(owner: T::AccountId) -> DispatchResult {
			...
			Self::deposit_event(Event::ShowOwnerKitties { owner, kitties: _kitties });
			Ok(())
		}

		pub fn do_display_kitties() -> DispatchResult {
			let mut _kitties: Vec<([u8; 16], Option<BalanceOf<T>>)> = Vec::new();
			for _kitty in Kitties::<T>::iter() {
				let (_, _kitty_infor) = _kitty;
				_kitties.push((_kitty_infor.dna, _kitty_infor.price));
			}
			if _kitties.len() == 0 {
				return Err(Error::<T>::NoKitty.into());
			}
            Self::deposit_event(Event::ShowKitties { kitties: _kitties });
			Ok(())
		}
	}
}
